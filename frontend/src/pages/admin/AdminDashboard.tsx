...
import { mockUsers, mockTrips, mockReservations } from '../../data/mockData';
...
const AdminDashboard: React.FC = () => {
  ...
  const [recentReservations, setRecentReservations] = useState(mockReservations.slice(0, 5));
  ...
  useEffect(() => {
    ...
    const activeTrips = mockTrips.filter(t => t.departureDate >= today).length;
    const completedTrips = mockTrips.filter(t => t.departureDate < today).length;
    const cancelledReservations = mockReservations.filter(r => r.status === 'cancelled').length;
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-8">
                ...
                return (
                  <div key={reservation.id} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                    ...
                    <div className="text-right">
                      <p className="font-semibold text-gray-900">₺{reservation.totalPrice}</p>
                      <p className="text-xs text-gray-500">
        ...
        <div className="bg-white rounded-lg shadow-sm border">
          <div className="p-6 border-b border-gray-200">
            <h2 className="text-xl font-semibold text-gray-900">Popüler Güzergahlar</h2>
          </div>
          <div className="p-6">
            <div className="space-y-4">
                ...
                <div key={route.route} className="flex items-center justify-between">
                  <div className="flex items-center space-x-3">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-gray-600">{route.count} rezervasyon</p>
                    </div>
                  </div>
                  <div className="w-20 bg-gray-200 rounded-full h-2">
                    <div 
                      className="bg-blue-600 h-2 rounded-full" 
                      style={{ width: `${(route.count / Math.max(...popularRoutes.map(r => r.count))) * 100}%` }}
                    ></div>
                  </div>
                </div>
              ))}
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="bg-white rounded-lg shadow-sm border p-6">
          <div className="flex items-center space-x-3">
            ...
            <div>
              <p className="text-sm font-medium text-gray-600">Tamamlanan Seferler</p>
        ...
        <div className="bg-white rounded-lg shadow-sm border p-6">
          <div className="flex items-center space-x-3">
            ...
            <div>
              <p className="text-sm font-medium text-gray-600">İptal Edilen Rezervasyonlar</p>
          ...
          <div className="flex items-center space-x-3">
            ...
            <div>
              ...
              <p className="text-2xl font-bold text-gray-900">
                ₺{Math.round(stats.totalRevenue / Math.max(stats.totalReservations, 1))}
              </p>
            </div>
          </div>
        </div>
